...
pub struct AgentSyncClient<OP> where OP: TOutputProtocol {
  _o_prot: OP,
  _sequence_number: i32,
}

impl <OP> AgentSyncClient<OP> where OP: TOutputProtocol {
  pub fn new(output_protocol: OP) -> AgentSyncClient<OP> {
    AgentSyncClient { _o_prot: output_protocol, _sequence_number: 0 }
  }
}

impl <OP> TThriftClient for AgentSyncClient<OP> where OP: TOutputProtocol {
  fn i_prot_mut(&mut self) -> &mut TInputProtocol { panic!("no input protocol") }
  fn o_prot_mut(&mut self) -> &mut TOutputProtocol { &mut self._o_prot }
  fn sequence_number(&self) -> i32 { self._sequence_number }
  fn increment_sequence_number(&mut self) -> i32 { self._sequence_number += 1; self._sequence_number }
}

impl <OP> TAgentSyncClientMarker for AgentSyncClient<OP> where OP: TOutputProtocol {}
    ...
    (
      {
        ...
        let call_args = EmitZipkinBatchArgs { spans: spans };
    ...
    (
      {
        ...
        let call_args = EmitBatchArgs { batch: batch };
        self.o_prot_mut().write_message_begin(&message_ident)?;
        call_args.write_to_out_protocol(self.o_prot_mut())?;
        self.o_prot_mut().write_message_end()?;
        self.o_prot_mut().flush()
...
impl <H: AgentSyncHandler> AgentSyncProcessor<H> {
  ...
  fn process_emit_zipkin_batch(&self, incoming_sequence_number: i32, i_prot: &mut TInputProtocol, o_prot: &mut TOutputProtocol) -> thrift::Result<()> {
    TAgentProcessFunctions::process_emit_zipkin_batch(&self.handler, incoming_sequence_number, i_prot, o_prot)
  }
  fn process_emit_batch(&self, incoming_sequence_number: i32, i_prot: &mut TInputProtocol, o_prot: &mut TOutputProtocol) -> thrift::Result<()> {
...
impl TAgentProcessFunctions {
  pub fn process_emit_zipkin_batch<H: AgentSyncHandler>(handler: &H, _: i32, i_prot: &mut TInputProtocol, _: &mut TOutputProtocol) -> thrift::Result<()> {
    let args = EmitZipkinBatchArgs::read_from_in_protocol(i_prot)?;
  ...
  pub fn process_emit_batch<H: AgentSyncHandler>(handler: &H, _: i32, i_prot: &mut TInputProtocol, _: &mut TOutputProtocol) -> thrift::Result<()> {
    let args = EmitBatchArgs::read_from_in_protocol(i_prot)?;
...
struct EmitZipkinBatchArgs {
  spans: Vec<zipkincore::Span>,
}

impl EmitZipkinBatchArgs {
  fn read_from_in_protocol(i_prot: &mut TInputProtocol) -> thrift::Result<EmitZipkinBatchArgs> {
    ...
    verify_required_field_exists("EmitZipkinBatchArgs.spans", &f_1)?;
    let ret = EmitZipkinBatchArgs {
...
#[derive(Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct EmitBatchArgs {
  pub batch: jaeger::Batch,
}

impl EmitBatchArgs {
  fn read_from_in_protocol(i_prot: &mut TInputProtocol) -> thrift::Result<EmitBatchArgs> {
    i_prot.read_struct_begin()?;
    ...
    i_prot.read_struct_end()?;
    verify_required_field_exists("EmitBatchArgs.batch", &f_1)?;
    let ret = EmitBatchArgs {
